#!/usr/bin/env python3
# ══════════════════════════════════════════════════
# جلال رادار v4.0 — Halal Professional Edition
# ══════════════════════════════════════════════════
...
DEFAULT_TADAWUL = {
    ...
    "5110":"الكهرباء","7030":"زين","4072":"MBC","2082":"أكوا باور",
    "4325":"مسار","4144":"رؤوم","4005":"رعاية","1831":"مهارة",
    "1833":"الموارد","1321":"أنابيب الشرق","4100":"مكة","2180":"فيبكو",
    "3008":"الكثيري","4040":"سابتكو","2230":"الكيميائية","4013":"سليمان الحبيب",
}
...
DEFAULT_US = {
    ...
    "COST":"Costco","HD":"Home Depot","QCOM":"Qualcomm","INTC":"Intel",
    "AMAT":"Applied Materials","LRCX":"Lam Research","KLAC":"KLA Corp",
    "MRVL":"Marvell Technology","AVGO":"Broadcom","TXN":"Texas Instruments",
    "MU":"Micron Technology","WM":"Waste Management","RSG":"Republic Services",
    "XOM":"ExxonMobil","CVX":"Chevron","COP":"ConocoPhillips",
    "EOG":"EOG Resources","PXD":"Pioneer Natural","OXY":"Occidental",
    "LIN":"Linde","APD":"Air Products","ECL":"Ecolab",
    "NKE":"Nike","SBUX":"Starbucks","MCD":"McDonald's",
    "DIS":"Disney","NFLX":"Netflix","PYPL":"PayPal",
    "UBER":"Uber","LYFT":"Lyft","ABNB":"Airbnb",
    "CRM":"Salesforce","NOW":"ServiceNow","SNOW":"Snowflake",
    "DDOG":"Datadog","ZS":"Zscaler","CRWD":"CrowdStrike",
    "PANW":"Palo Alto","FTNT":"Fortinet","NET":"Cloudflare",
}
...
SECTORS_US = {
    "تكنولوجيا 💻":     ["AAPL","MSFT","GOOGL","META","NVDA","AMD","QCOM","INTC","AMAT","LRCX","KLAC","MRVL","AVGO","TXN","MU","CRM","NOW","SNOW","DDOG"],
    "أمن سيبراني 🔒":   ["ZS","CRWD","PANW","FTNT","NET"],
    "طاقة متجددة ☀️":   ["ENPH","FSLR","NEE"],
    "صحة 🏥":           ["JNJ","UNH","ABBV"],
    "استهلاك 🛒":       ["COST","HD","NKE","SBUX","MCD","DIS","NFLX"],
    "طاقة تقليدية ⛽":  ["XOM","CVX","COP","EOG","OXY"],
    "كيماويات 🧪":      ["LIN","APD","ECL"],
    "تقنية مالية 📱":   ["PYPL","UBER","LYFT","ABNB"],
}
SECTORS_CRYPTO = {
...
def backtest_stock(code, market="tadawul", lookback_days=180):
    """اختبر الاستراتيجية على آخر 6 أشهر"""
    try:
        ...
        trades = []
        # نمسح كل يوم في الفترة الماضية
        for i in range(50, len(df)-lookback_days, 5):  # كل 5 أيام
            window = df.iloc[:i+lookback_days].copy()
            if len(window) < 50: continue
            price = float(window["Close"].iloc[-1])

            # نطبق نفس منطق الاستراتيجية
            ...
            score=(2 if c1 else 0)+(2 if c2 else 0)+(2 if c3 else 0)+(2 if c4 else 0)+                  (2 if c6 else 0)+(2 if c8 else 0)+(2 if c9 else 0)+(2 if c10 else 0)
            ...
            if score >= 13 and av > 0:
                ...
                # تتبع النتيجة في الأيام التالية
...
def run_backtest(market):
    """تشغيل backtest على أفضل 10 أسهم"""
    custom = load_custom()
    if market == "tadawul":
    ...
    results = []
    sample = list(stocks.items())[:15]  # أول 15 للسرعة
...
def analyze(code,name,market,bdf=None):
    try:
        ...
        liq_lbl=("عالية 🟢 "+str(liq)+" م.ر") if liq>100 else (("متوسطة 🟡 "+str(liq)+" م.ر") if liq>10 else ("منخفضة 🔴 "+str(liq)+" م.ر"))
        ...
        return {
            ...
            "liq":liq,"liq_lbl":liq_lbl,"rsv":rsv,"rsl":rsl,
...
def check_and_notify(results, market):
    """تحقق من نتائج المسح وأرسل تنبيهات للإشارات القوية"""
    cfg = load_telegram()
    ...
    for s in results:
        ...
        key = s["code"]+"-"+today
        if key in last_sent: continue  # ما نرسل نفس الإشارة مرتين في اليوم
...
def run_scan(market):
    ...
    def scan_one(code,name):
        if market=="crypto":
            ...
            r=analyze(code,name,market,bdf)
        with lock:
            done[0]+=1
    ...
    # مسح متوازٍ — 8 أسهم في نفس الوقت
...
def build_row(s,i):
    ...
    detail=(
        ...
        '<div class="dqi"><div class="dql">السيولة اليومية 💧</div><div class="dqv">'+s.get("liq_lbl","—")+'</div><div class="dqh">م = مليون ريال</div></div>'
...
:root{
  --bg:#070b14;--card:#0d1525;--card2:#111e33;--bdr:#1a2d4a;
  --gold:#d4a843;--gold2:#f0c060;--green:#22c55e;--red:#ef4444;
  --yel:#eab308;--blue:#3b82f6;--teal:#06b6d4;--purple:#8b5cf6;
  --txt:#e2e8f0;--mut:#64748b;--mut2:#94a3b8;
}
...
body::before{content:'';position:fixed;inset:0;
  background:
    radial-gradient(ellipse 800px 600px at 10% 20%,rgba(212,168,67,0.04),transparent),
    radial-gradient(ellipse 600px 400px at 90% 80%,rgba(6,182,212,0.04),transparent),
    radial-gradient(ellipse 400px 400px at 50% 50%,rgba(59,130,246,0.02),transparent);
...
.hdr::after{content:'';position:absolute;bottom:0;left:50%;transform:translateX(-50%);
  width:200px;height:1px;background:linear-gradient(90deg,transparent,var(--gold),transparent);}
.logo{font-size:clamp(1.6rem,5vw,2.8rem);font-weight:700;letter-spacing:3px;
  background:linear-gradient(135deg,var(--gold) 0%,var(--gold2) 40%,var(--teal) 100%);
  -webkit-background-clip:text;-webkit-text-fill-color:transparent;margin-bottom:6px;}
.logo-sub{color:var(--mut);font-size:0.75rem;letter-spacing:1px;text-transform:uppercase;}
.sess-bar{display:inline-flex;align-items:center;gap:6px;margin-top:10px;
  background:var(--card);border:1px solid var(--bdr);border-radius:20px;
  padding:4px 14px;font-size:0.72rem;color:var(--mut2);}

/* ══ Tabs ══ */
.tabs{display:flex;gap:4px;background:var(--card);border:1px solid var(--bdr);
  border-radius:12px;padding:4px;margin-bottom:16px;}
.tab-btn{flex:1;padding:8px 12px;border-radius:8px;border:none;background:transparent;
  color:var(--mut);font-family:inherit;font-size:0.85rem;cursor:pointer;transition:all 0.2s;text-align:center;}
.tab-btn:hover{color:var(--txt);}
.tab-btn.active{background:linear-gradient(135deg,#1a3060,#0f1e40);color:var(--gold2);
...
.scan-btn{display:inline-flex;align-items:center;gap:8px;
  background:linear-gradient(135deg,#1a3060 0%,#0f1e40 100%);
  border:1px solid var(--gold);color:var(--gold2);
  padding:10px 28px;border-radius:50px;font-size:0.9rem;font-family:inherit;
...
/* ══ Progress ══ */
.prog-wrap{background:var(--card);border:1px solid var(--bdr);border-radius:8px;padding:8px 12px;margin-bottom:12px;display:none;}
.prog-wrap.show{display:block;}
.prog-bar{height:4px;background:rgba(255,255,255,0.06);border-radius:4px;overflow:hidden;margin-top:5px;}
.prog-fill{height:100%;background:linear-gradient(90deg,var(--gold),var(--teal));border-radius:4px;transition:width 0.5s;}
.prog-txt{font-size:0.7rem;color:var(--mut2);}

/* ══ Stats Chips ══ */
.chips{display:flex;gap:6px;flex-wrap:wrap;margin-bottom:14px;}
.chip{flex:1;min-width:72px;background:var(--card);border:1px solid var(--bdr);
  border-radius:10px;padding:8px 10px;text-align:center;cursor:pointer;
  transition:all 0.2s;}
.chip:hover{transform:translateY(-1px);}
.chip.af{transform:translateY(-2px);}
.chip .num{font-size:1.4rem;font-weight:700;line-height:1;}
.chip .lbl{font-size:0.61rem;color:var(--mut);margin-top:3px;}
.cg1{border-color:rgba(212,168,67,0.3)}.cg1 .num{color:var(--gold)}
.cg1.af{background:rgba(212,168,67,0.07);}
.cg2{border-color:rgba(34,197,94,0.3)}.cg2 .num{color:var(--green)}
.cg2.af{background:rgba(34,197,94,0.07);}
.cg3{border-color:rgba(234,179,8,0.3)}.cg3 .num{color:var(--yel)}
.cg3.af{background:rgba(234,179,8,0.07);}
.cg4{border-color:rgba(239,68,68,0.3)}.cg4 .num{color:var(--red)}
.cg4.af{background:rgba(239,68,68,0.07);}
.cg5{border-color:rgba(6,182,212,0.3)}.cg5 .num{color:var(--teal)}
.cg5.af{background:rgba(6,182,212,0.07);}
.cg6{border-color:rgba(34,197,94,0.4)}.cg6 .num{color:var(--green)}
.cg6.af{background:rgba(34,197,94,0.1);}

/* ══ Explosion Section ══ */
.exp-sec{background:rgba(212,168,67,0.04);border:1px solid rgba(212,168,67,0.15);
  border-radius:11px;padding:11px;margin-bottom:13px;}
...
.exp-card{background:rgba(0,0,0,0.2);border:1px solid rgba(212,168,67,0.12);border-radius:8px;
  padding:7px 10px;display:flex;align-items:center;justify-content:space-between;gap:6px;}
.exp-nm{font-weight:600;font-size:0.83rem;}.exp-cd{color:var(--mut);font-size:0.63rem;}
.exp-px{color:var(--gold2);font-weight:700;font-size:0.86rem;}
.exp-rt{background:rgba(212,168,67,0.1);color:var(--yel);padding:2px 7px;border-radius:6px;font-size:0.67rem;font-weight:600;}
...
.heat-nm{font-size:0.76rem;font-weight:600;margin-bottom:5px;}
.heat-sc{font-size:1.25rem;font-weight:700;margin-bottom:4px;}
.heat-bar{height:3px;border-radius:3px;margin:0 auto 4px;max-width:80%;}
.heat-inf{font-size:0.6rem;color:rgba(255,255,255,0.5);}

/* ══ Stock List ══ */
.slist{display:flex;flex-direction:column;gap:4px;}
.srow{background:var(--card);border:1px solid var(--bdr);border-radius:10px;
...
.srow.buy{border-right:2px solid var(--green);}
.srow.wait{border-right:2px solid var(--yel);}
.srow.avoid{border-right:2px solid var(--red);}
.sr-l{display:flex;align-items:center;gap:6px;min-width:0;flex:1;}
...
.sr-code{color:var(--mut);font-size:0.62rem;flex-shrink:0;}
.sr-stars{color:var(--gold);font-size:0.74rem;}
.sr-tr{color:var(--mut);font-size:0.67rem;}
...
.sc-hi{color:var(--green);background:rgba(34,197,94,0.1);}
.sc-md{color:var(--yel);background:rgba(234,179,8,0.1);}
.sc-lo{color:var(--red);background:rgba(239,68,68,0.1);}
.sr-fr{font-size:0.72rem;display:flex;gap:2px;font-weight:600;}
.sr-rsi{color:var(--mut);font-size:0.66rem;}
.sr-chev{color:var(--mut);font-size:0.65rem;transition:transform 0.25s;}
...
.rank-badge{display:inline-flex;align-items:center;justify-content:center;
  width:24px;height:24px;border-radius:50%;font-size:0.78rem;font-weight:700;flex-shrink:0;}
...
/* ══ Detail Card ══ */
.det{display:none;background:rgba(0,0,0,0.15);border:1px solid var(--bdr);
  border-radius:0 0 10px 10px;padding:13px;margin-top:-4px;border-top:none;}
.det.op{display:block;}

/* Why Box */
.why-box{background:rgba(59,130,246,0.05);border:1px solid rgba(59,130,246,0.15);
...
.why-tag{background:rgba(0,0,0,0.2);border:1px solid var(--bdr);border-radius:6px;
  padding:2px 8px;font-size:0.62rem;color:var(--txt);}
...
.dql{font-size:0.58rem;color:var(--mut);margin-bottom:2px;font-weight:500;}
.dqv{font-size:0.78rem;font-weight:600;}.dqh{font-size:0.52rem;color:var(--mut);margin-top:2px;}
...
.dt{font-size:0.63rem;color:var(--mut2);font-weight:600;margin-bottom:7px;letter-spacing:0.3px;}
...
.cv:hover{background:rgba(212,168,67,0.12);}
.cv:active{transform:scale(0.92);}
.copied{background:rgba(34,197,94,0.15)!important;color:var(--green)!important;}
.rr-row{display:flex;justify-content:space-between;font-size:0.66rem;color:var(--mut);
  margin-top:7px;padding-top:6px;border-top:1px solid var(--bdr);}
.tr-note{font-size:0.6rem;color:var(--teal);margin-top:5px;padding-top:5px;
  border-top:1px solid rgba(6,182,212,0.15);}
.peg{display:grid;grid-template-columns:1fr 1fr 1fr;gap:5px;margin-bottom:7px;}
.pei{text-align:center;background:rgba(0,0,0,0.18);border-radius:6px;padding:5px;}
.pel{font-size:0.57rem;color:var(--mut);}.pev{font-size:0.88rem;font-weight:700;margin-top:1px;}
.be-n{font-size:0.62rem;color:var(--teal);background:rgba(6,182,212,0.04);
  padding:5px 8px;border-radius:5px;border:1px solid rgba(6,182,212,0.1);}
.pro-exp{font-size:0.62rem;color:var(--mut);background:rgba(59,130,246,0.04);
  padding:4px 7px;border-radius:5px;margin-bottom:6px;border-right:2px solid var(--blue);}
.cond-note{font-size:0.63rem;color:var(--yel);background:rgba(234,179,8,0.05);
  padding:3px 7px;border-radius:5px;margin-bottom:7px;border-right:2px solid var(--yel);}
.ob-row{display:flex;justify-content:space-between;align-items:flex-start;
  padding:3px 5px;border-radius:5px;background:rgba(59,130,246,0.04);margin-bottom:3px;font-size:0.66rem;}
.ob-z{color:var(--blue)}.ob-m{color:var(--mut);font-size:0.6rem;}.ob-d{font-size:0.57rem;color:var(--mut);margin-top:1px;}
.cb{background:none;border:1px solid var(--bdr);color:var(--mut);padding:4px 9px;
  border-radius:5px;font-family:inherit;font-size:0.63rem;cursor:pointer;width:100%;margin-top:3px;}
...
.ci{display:flex;align-items:center;gap:3px;font-size:0.6rem;padding:2px 4px;
...
.cw{color:var(--gold);font-size:0.55rem;margin-right:auto;}
.btn-row{display:flex;gap:5px;margin-top:7px;}
.btn-del{background:rgba(239,68,68,0.07);border:1px solid rgba(239,68,68,0.2);color:var(--red);
  padding:4px 9px;border-radius:5px;font-family:inherit;font-size:0.63rem;cursor:pointer;}
.btn-excl{background:rgba(100,116,139,0.07);border:1px solid rgba(100,116,139,0.2);color:var(--mut);
  padding:4px 9px;border-radius:5px;font-family:inherit;font-size:0.63rem;cursor:pointer;}
...
.ci-inp{background:rgba(0,0,0,0.3);border:1px solid var(--bdr);color:var(--txt);
  padding:6px 9px;border-radius:7px;font-family:inherit;font-size:0.77rem;flex:1;min-width:80px;}
.ci-inp:focus{outline:none;border-color:var(--gold);}
.add-btn{background:rgba(34,197,94,0.09);border:1px solid rgba(34,197,94,0.3);color:var(--green);
  padding:6px 12px;border-radius:7px;cursor:pointer;font-family:inherit;font-size:0.77rem;}
.file-row{display:flex;align-items:center;gap:7px;margin-top:5px;}
.file-lbl{background:rgba(59,130,246,0.09);border:1px solid rgba(59,130,246,0.25);color:var(--blue);
  padding:5px 11px;border-radius:7px;cursor:pointer;font-size:0.73rem;white-space:nowrap;}
.file-hint{font-size:0.64rem;color:var(--mut);}
.ecl{display:inline-block;background:rgba(239,68,68,0.06);border:1px solid rgba(239,68,68,0.18);
  color:var(--red);padding:2px 7px;border-radius:10px;font-size:0.63rem;cursor:pointer;margin:2px;}

/* Glossary */
.gloss{background:var(--card);border:1px solid var(--bdr);border-radius:11px;padding:11px;margin-bottom:13px;}
.gloss-ttl{font-size:0.8rem;font-weight:600;color:var(--teal);cursor:pointer;
...
.gi-en{font-size:0.73rem;font-weight:700;color:var(--blue);}
.gi-ar{font-size:0.7rem;color:var(--txt);margin-top:1px;}
.gi-desc{font-size:0.62rem;color:var(--mut);margin-top:2px;line-height:1.4;}

/* Loading */
.lo{position:fixed;inset:0;background:rgba(7,11,20,0.95);display:none;
  flex-direction:column;align-items:center;justify-content:center;z-index:100;}
.lo.show{display:flex;}
.lo-spin{width:52px;height:52px;border:2px solid var(--bdr);border-top-color:var(--gold);
...
.toast{position:fixed;bottom:20px;left:50%;transform:translateX(-50%);
  ...
  padding:6px 16px;border-radius:14px;font-size:0.74rem;font-weight:600;
...
.bt-sv{font-size:1.1rem;font-weight:700;}.bt-sl{font-size:0.58rem;color:var(--mut);}
.bt-grade{display:inline-block;padding:2px 8px;border-radius:6px;font-size:0.72rem;font-weight:700;}
.bg-A{background:rgba(34,197,94,0.15);color:var(--green);}
.bg-B{background:rgba(212,168,67,0.15);color:var(--gold2);}
.bg-C{background:rgba(234,179,8,0.15);color:var(--yel);}
.bg-D{background:rgba(239,68,68,0.15);color:var(--red);}
/* Telegram Settings */
...
/* No data */
.nr{text-align:center;padding:50px 20px;color:var(--mut);}
.nr-icon{font-size:2.5rem;margin-bottom:10px;}
...
@media(max-width:768px){
  ...
  .exp-grid{grid-template-columns:1fr;}.heat-grid{grid-template-columns:repeat(2,1fr);}
  .tabs{gap:2px;}.tab-btn{padding:7px 8px;font-size:0.78rem;}
  .gloss-grid{grid-template-columns:1fr;}
  .sr-name{max-width:85px;}
}
@media(max-width:400px){
  .sr-sc{display:none;}.og2{grid-template-columns:1fr;}.tabs{flex-wrap:wrap;}
}
</style>
"""
...
function toggle(mkt,i){
  var det=document.getElementById('det-'+mkt+'-'+i);
  var ch=document.getElementById('ch-'+mkt+'-'+i);
  if(!det) return;
  if(det.classList.contains('op')){det.classList.remove('op');if(ch)ch.classList.remove('op');}
  else{det.classList.add('op');if(ch)ch.classList.add('op');}
}
...
function filterChip(mkt,ft,el){
  ...
  if(aFilt[mkt]===ft){
    ...
    rows.forEach(r=>{
      ...
      if(ft==='halal'&&r.dataset.halal==='1') show=true;
...
function startScan(){
  var lb={tadawul:'تاسي',us:'السوق الأمريكي',crypto:'العملات'};
  document.getElementById('lo').classList.add('show');
  document.getElementById('lm').textContent=lb[aTab]||aTab;
  document.getElementById('lo-pfill').style.width='0%';
  fetch('/scan?market='+aTab).then(r=>r.json()).then(()=>{
    scanPoll=setInterval(pollProgress,1500);
  });
}
...
function pollProgress(){
  fetch('/status?market='+aTab).then(r=>r.json()).then(d=>{
    ...
    document.getElementById('lo-pfill').style.width=pct+'%';
    document.getElementById('lo-sub').textContent='تم مسح '+d.progress+'% — '+d.total+' سهم';
    if(d.status==='done'){
...
function copyVal(el){
  ...
  navigator.clipboard.writeText(t).then(()=>{
    el.classList.add('copied'); showToast('✓ '+t);
...
function uploadFile(){
  ...
  reader.onload=function(e){
    ...
    if(!stocks.length){showToast('ما في بيانات');return;}
    fetch('/add_bulk',{method:'POST',headers:{'Content-Type':'application/json'},
      body:JSON.stringify({stocks,market:mkt})})
    .then(r=>r.json()).then(d=>{showToast('تمت إضافة '+d.count+' ✓');setTimeout(()=>location.reload(),1000);});
  };
...
// ══ Backtest ══
function runBacktest(mkt){
  showToast('جاري اختبار الاستراتيجية...');
  fetch('/backtest?market='+mkt).then(r=>r.json()).then(()=>{
    setTimeout(()=>loadBacktest(mkt),3000);
  });
}
function loadBacktest(mkt){
  fetch('/backtest_results?market='+mkt).then(r=>r.json()).then(d=>{
    if(!d.data||!d.data.length){showToast('جاري الحساب...');setTimeout(()=>loadBacktest(mkt),5000);return;}
    var html='';
    d.data.forEach(function(s){
      var gc='bg-'+s.grade;
      var wr_cls=s.win_rate>=65?'pos':(s.win_rate>=50?'teal-t':'neg');
      html+='<div class="bt-card">';
      html+='<div class="bt-name" style="display:flex;justify-content:space-between;">';
      html+=s.name+'<span class="bt-grade '+gc+'">'+s.grade+'</span></div>';
      html+='<div class="bt-stats">';
      html+='<div class="bt-stat"><div class="bt-sv '+wr_cls+'">'+s.win_rate+'%</div><div class="bt-sl">نسبة النجاح</div></div>';
      html+='<div class="bt-stat"><div class="bt-sv">'+s.trades+'</div><div class="bt-sl">صفقة مختبرة</div></div>';
      html+='<div class="bt-stat"><div class="bt-sv pos">+'+s.avg_win+'%</div><div class="bt-sl">متوسط الربح</div></div>';
      html+='<div class="bt-stat"><div class="bt-sv neg">'+s.avg_loss+'%</div><div class="bt-sl">متوسط الخسارة</div></div>';
      html+='</div></div>';
    });
    document.getElementById('bt-results-'+mkt).innerHTML=html;
    document.getElementById('bt-ts-'+mkt).textContent='آخر اختبار: '+d.ts;
  });
}
...
function loadTelegramConfig(){
  fetch('/telegram_config').then(r=>r.json()).then(d=>{
    ...
    document.getElementById('tg-rank').value=d.min_rank||70;
...
function saveTelegramConfig(){
  var data={
    ...
    min_rank:document.getElementById('tg-rank').value,
    enabled:document.getElementById('tg-enabled').checked
  };
  fetch('/telegram_config',{method:'POST',headers:{'Content-Type':'application/json'},
    body:JSON.stringify(data)})
  .then(r=>r.json()).then(d=>{if(d.ok) showToast('تم الحفظ ✓');});
}
function testTelegram(){
  fetch('/test_telegram').then(r=>r.json()).then(d=>{
    showToast(d.ok?'✅ وصلت الرسالة!':'❌ تحقق من الإعدادات');
  });
}
...
GLOSSARY = [
    ...
    ("Partial Exit","خروج جزئي","50% عند TP1، 30% عند TP2، 20% اترك تجري مع الترند"),
    ("Relative Strength","القوة النسبية","السهم أقوى من السوق = فرصة أفضل حتى لو السوق ضعيف"),
    ("Sector Rotation","دوران القطاعات","الأموال تنتقل من قطاع لآخر — القطاع الساخن 🔥 هو الفرصة"),
    ("BUY مشروط","دخول مع تحفظ","الشروط الأساسية متحققة لكن فيه نقطة ضعف — راجع التحفظ"),
    ("Score 15+","إشارة قوية","15+ من 20 = معظم الشروط متحققة ✅"),
    ("م.ر","مليون ريال","وحدة قياس السيولة اليومية"),
]
...
def index():
    custom=load_custom(); excl=custom.get("excluded",[])
    so,sl=is_us_session()
    sc="rgba(34,197,94,0.08)" if so else "rgba(239,68,68,0.06)"
    ...
    out.append('<title>جلال رادار v4</title>')
    out.append(CSS)
    ...
    out.append('<div class="logo">⚡ جلال رادار</div>')
    out.append('<div class="logo-sub">Jalal Radar v4.0 — Halal Professional Edition</div>')
    out.append('<div class="sess-bar">🕐 '+sl+'</div>')
    out.append('</div>')

    # Tabs
    out.append('<div class="tabs">')
    out.append('<button class="tab-btn active" id="tb-tadawul" onclick="switchTab(\'tadawul\')">🇸🇦 تاسي</button>')
    out.append('<button class="tab-btn" id="tb-us" onclick="switchTab(\'us\')">🇺🇸 أمريكي</button>')
    out.append('<button class="tab-btn" id="tb-crypto" onclick="switchTab(\'crypto\')">💰 عملات</button>')
    out.append('</div>')

    # Glossary
    out.append('<div class="gloss">')
    out.append('<div class="gloss-ttl" onclick="toggleGloss()">📖 دليل المصطلحات <span id="gloss-chev">▼</span></div>')
    ...
    # Telegram Settings
    out.append('<div class="tg-sec">')
    out.append('<div class="tg-ttl">📲 إعدادات تنبيهات Telegram</div>')
    ...
    out.append('<div class="tg-row"><span class="tg-lbl">حد الترجيح:</span><input class="tg-inp" id="tg-rank" type="number" value="70" style="max-width:80px;"> <span style="font-size:0.65rem;color:var(--mut);">يرسل فقط للإشارات التي نقطتها أكبر من هذا</span></div>')
    ...
    out.append('<div class="tg-row"><button class="tg-save" onclick="saveTelegramConfig()">💾 حفظ</button><button class="tg-test" onclick="testTelegram()">🧪 اختبار</button></div>')
    out.append('</div>')
    ...
    for mkt,label,flag in [("tadawul","تاسي","🇸🇦"),("us","السوق الأمريكي","🇺🇸"),("crypto","العملات الرقمية","💰")]:
        ...
        out.append('<div class="scan-wrap">')
        ...
        if data:
            ...
            if heat:
                ...
                out.append('<div class="heat-sec"><div class="heat-ttl">🌡️ خريطة القطاعات — أين تتجمع الفلوس؟</div><div class="heat-grid">')
                for sector,info in heat.items():
                    ...
                    if rank<top_t:
                        bg="rgba(34,197,94,0.1)";bc="rgba(34,197,94,0.3)";sc2="var(--green)";lbl="🔥 الأقوى"
                    elif rank<top_t*2:
                        bg="rgba(234,179,8,0.08)";bc="rgba(234,179,8,0.25)";sc2="var(--yel)";lbl="👀 متوسط"
                    else:
                        bg="rgba(239,68,68,0.07)";bc="rgba(239,68,68,0.2)";sc2="var(--red)";lbl="❄️ الأضعف"
                    pct=min(int(avg/20*100),100)
                    out.append('<div class="heat-card" style="background:'+bg+';border:1px solid '+bc+';">')
                    out.append('<div class="heat-nm">'+sector+'</div>')
                    out.append('<div class="heat-sc" style="color:'+sc2+'">'+str(avg)+'</div>')
                    out.append('<div class="heat-bar" style="background:'+sc2+';width:'+str(pct)+'%;"></div>')
                    out.append('<div class="heat-inf">'+lbl+' | '+str(buy)+' BUY من '+str(cnt)+'</div>')
                    out.append('</div>')
                out.append('</div></div>')
        ...
        # Backtest section
        out.append('<div class="bt-sec">')
        out.append('<div class="bt-ttl"><span>📊 Backtest — اختبار الاستراتيجية على بيانات حقيقية</span><button class="bt-btn" onclick="runBacktest(\''+mkt+'\');">🔬 ابدأ الاختبار</button></div>')
        out.append('<div style="font-size:0.65rem;color:var(--mut);margin-bottom:8px;">يختبر الاستراتيجية على آخر 6 أشهر ويحسب نسبة النجاح لكل سهم</div>')
        out.append('<span class="scan-ts" id="bt-ts-'+mkt+'"></span>')
        out.append('<div class="bt-grid" id="bt-results-'+mkt+'"></div>')
        out.append('</div>')
        out.append('</div>')
...
def backtest_route():
    ...
    t = threading.Thread(target=run_backtest, args=(market,))
...
def test_telegram():
    ok = send_telegram("✅ جلال رادار — الاتصال يعمل بنجاح!")
...
if __name__=="__main__":
    ...
    print("="*55)
    print("  ⚡ جلال رادار v4.0 — Halal Professional")
    print("  تاسي: "+str(len(DEFAULT_TADAWUL))+" سهم شرعي")
