...
def row_html(s,i):
    ...
    row=(
        ...
        '<div class="sr-l">'
        + ('<span class="rank-badge rank-'+('1' if s.get('rank_pos',99)==1 else '2' if s.get('rank_pos',99)==2 else '3' if s.get('rank_pos',99)==3 else 'other')+'">'+s.get('rank_medal','')+'</span>' if s.get('rank_pos') else '')
        ...
        '<div class="sr-r">'
        + ('<span class="rank-score">'+str(s.get('rank_score',''))+'pt</span>' if s.get('rank_score') else '')
    ...
    pos = s.get("rank_pos", 0)
    medal = s.get("rank_medal", "")
    rs = s.get("rank_score", 0)
    if pos and reasons:
        why_html = '<div class="why-section">'
        why_html += '<div class="why-title">'+medal+' السبب في ترتيبه #'+str(pos)+' — نقطة الترجيح: '+str(rs)+'</div>'
        why_html += '<div class="why-list">'
        for r in reasons:
            cls = "why-item warn" if "⚠️" in r else "why-item"
            why_html += '<span class="'+cls+'">'+r+'</span>'
        why_html += '</div></div>'
