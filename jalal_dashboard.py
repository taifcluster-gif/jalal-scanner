...
# ══ قوائم الحلال ══
HALAL_TADAWUL_EXCLUDE = {
    # بنوك (ربا)
    "1010","1020","1030","1050","1060","1080","1120","1140","1150","1180",
    # تأمين
    "8010","8020","8030","8040","8050","8060","8070","8080","8100","8120","8150","8160","8170","8180","8190","8200","8210","8230","8240","8250","8260","8270","8280","8300","8310",
}

HALAL_US = {
    "AAPL","MSFT","GOOGL","META","NVDA","AMD","TSLA","AMZN",
    "ENPH","FSLR","NEE","JNJ","ABBV","COST","HD",
}

HALAL_CRYPTO = {
    "BTCUSDT","ETHUSDT","BNBUSDT","SOLUSDT","ADAUSDT",
    "DOTUSDT","LINKUSDT","AVAXUSDT","MATICUSDT","ATOMUSDT",
    "XLMUSDT","ALGOUSDT","VETUSDT","UNIUSDT","SANDUSDT","MANAUSDT",
}

def is_halal(code, market):
    if market == "tadawul":
        return code not in HALAL_TADAWUL_EXCLUDE
    elif market == "us":
        return code in HALAL_US
    elif market == "crypto":
        return code in HALAL_CRYPTO
    return False
...
# قطاعات السوق الأمريكي
SECTORS_US = {
    "تكنولوجيا 💻":   ["AAPL","MSFT","GOOGL","META","NVDA","AMD"],
...
# قطاعات العملات الرقمية
SECTORS_CRYPTO = {
    "Layer 1 ⛓️":     ["BTCUSDT","ETHUSDT","SOLUSDT","ADAUSDT","AVAXUSDT"],
    "DeFi 🏦":        ["UNIUSDT","AAVEUSDT","LINKUSDT"],
    "الميتافيرس 🌐":  ["SANDUSDT","MANAUSDT"],
    "خدمات 🔧":       ["BNBUSDT","XRPUSDT","LTCUSDT","XLMUSDT"],
    "بنية تحتية 🔩":  ["DOTUSDT","MATICUSDT","ATOMUSDT","ALGOUSDT","VETUSDT","FILUSDT"],
}

SECTORS_MAP = {
    "tadawul": SECTORS_TADAWUL,
    "us":      SECTORS_US,
    "crypto":  SECTORS_CRYPTO,
}
...
def analyze(code,name,market,bdf=None):
    try:
        ...
        return {
            ...
            "conds":conds,"cok":sum(1 for c in conds if c["ok"]),"halal":is_halal(code,market),"is_custom":False,
        }
    except: return None
...
def analyze_crypto(sym,name,bdf=None):
    try:
        ...
        adx_val=adx_f(d["High"],d["Low"],d["Close"])
        av=atr_f(d["High"],d["Low"],d["Close"])
        atr_v=float(av.iloc[-1]); adx_v=round(float(adx_val.iloc[-1]),1); p=round(price,4)
        ...
        return {
            ...
            "lb":lb,"t1s":t1s,"t1l":t1l,"t2s":t2s,"t2l":t2l,"ss":ss,"sl":sl,
            "rr":rr,"psl":psl,"ptp":ptp,"tp":tp,"tg":tg,"be":be,
            "dur":dur(trend,atr_v,p,t1l),"rsi":round(float(rv.iloc[-1]),1),"adx":adx_v,
            "vr":round(abs(chg)/3,1) if abs(chg)>3 else 1,"exp":chg>10,
            "liq":round(info.get("vol_usdt",0)/1e6,1),
            "liq_lbl":("عالية 🟢 "+str(round(info.get("vol_usdt",0)/1e6,1))+" م.ر") if info.get("vol_usdt",0)>100e6 else (("متوسطة 🟡 "+str(round(info.get("vol_usdt",0)/1e6,1))+" م.ر") if info.get("vol_usdt",0)>10e6 else ("منخفضة 🔴 "+str(round(info.get("vol_usdt",0)/1e6,1))+" م.ر")),
            "rsv":rsv,"rsl":rsl,
            "obs":find_obs(dfd),"fvgs":find_fvgs(dfd),
            "conds":[],"cok":0,"halal":is_halal(sym,"crypto"),"is_custom":False,
        }
    except: return None
...
def run_scan(market):
    ...
    # ══ نظام الترجيح النهائي ══
    def calc_rank_score(s):
        rank = 0
        reasons = []

        # 1. JRF Score (0-20 نقطة) — وزن 40%
        jrf = s.get("score", 0)
        rank += jrf * 2
        ...
        # 3. القوة النسبية — وزن 15%
        rsv = s.get("rsv", 0)
        if rsv > 5:   rank += 15; reasons.append("أقوى من السوق بكثير 📈")
        elif rsv > 2: rank += 10; reasons.append("أقوى من السوق 📈")
        elif rsv > 0: rank += 5

        # 4. ADX — وزن 10%
        adx = s.get("adx", 0)
        if adx > 40:  rank += 10; reasons.append("ترند قوي جداً ADX "+str(adx)+" 🔥")
        elif adx > 25: rank += 6; reasons.append("ترند جيد ADX "+str(adx))
        elif adx > 20: rank += 3

        # 5. السيولة — وزن 5%
        liq = s.get("liq", 0)
        if liq > 100:  rank += 5; reasons.append("سيولة عالية 💧")
        elif liq > 10: rank += 2

        # 6. حجم شاذ — وزن 5%
        if s.get("exp"): rank += 8; reasons.append("حجم تداول شاذ 🚀 ×"+str(s.get("vr",1)))

        # 7. R:R
        rr = s.get("rr", 0)
        if rr >= 2.5:  rank += 5; reasons.append("R:R ممتاز "+str(rr))
        elif rr >= 1.5: rank += 3

        # خصم للـ BUY مشروط
        if "مشروط" in s.get("bt",""):
            rank -= 5
            reasons.append("⚠️ BUY مشروط — راجع التحفظ")

        s["rank_score"] = round(rank, 1)
        s["rank_reasons"] = reasons
        return rank

    # حساب الترجيح لكل نتيجة
    for s in results:
        calc_rank_score(s)

    # ترتيب: BUY أولاً ثم حسب rank_score
    results.sort(key=lambda x:(x["priority"], -x.get("rank_score",0)))

    # إضافة الترتيب والميدالية
    rank_medals = ["🥇","🥈","🥉","4️⃣","5️⃣","6️⃣","7️⃣","8️⃣","9️⃣","🔟"]
    for i,s in enumerate(results):
        s["rank_pos"] = i+1
        s["rank_medal"] = rank_medals[i] if i < len(rank_medals) else str(i+1)
...
def row_html(s,i):
    ...
    # ترتيب الميدالية
    rpos = s.get('rank_pos', 99)
    medal = s.get('rank_medal', '')
    rsc = s.get('rank_score', 0)
    rnk_cls = 'rank-1' if rpos==1 else ('rank-2' if rpos==2 else ('rank-3' if rpos==3 else 'rank-other'))
    ...
    row=(
        ...
        +('<span class="halal-badge" title="حلال ✅">☪️</span>' if s.get('halal') else '<span class="haram-badge" title="يُنصح بمراجعة الحكم الشرعي">⚠️</span>')
        ...
        +'<span class="rank-badge '+rnk_cls+'">'+medal+'</span>'
    ...
    # قسم لماذا هذا الترتيب
    # قسم لماذا هذا الترتيب — يُبنى هنا ويُضاف في أول التفاصيل
    why_html=""
    _reasons = s.get("rank_reasons", [])
    _pos = s.get("rank_pos", 0)
    _medal = s.get("rank_medal", "")
    _rs = s.get("rank_score", 0)
    if _pos and _reasons:
        why_html = '<div class="why-section">'
        why_html += '<div class="why-title">'+_medal+' السبب في ترتيبه #'+str(_pos)+' — نقطة الترجيح: '+str(_rs)+'</div>'
        why_html += '<div class="why-list">'
        for _r in _reasons:
            _cls = "why-item warn" if "⚠️" in _r else "why-item"
            why_html += '<span class="'+_cls+'">'+_r+'</span>'
        why_html += '</div></div>'
    ...
    detail=(
        ...
        '<div class="dqi"><div class="dql">السيولة اليومية 💧</div><div class="dqv">'+s.get("liq_lbl","—")+'</div><div class="dql" style="font-size:0.54rem;color:var(--mut);margin-top:1px;">متوسط التداول اليومي (م = مليون ريال)</div></div>'
...
/* Halal */
.halal-badge{font-size:0.75rem;margin-right:2px;cursor:help;}
.haram-badge{font-size:0.65rem;margin-right:2px;cursor:help;opacity:0.5;}
.chip.ch{border-color:#2ecc71;}.chip.ch .num{color:#2ecc71;}.chip.ch.af{background:rgba(46,204,113,0.08);}
/* Ranking */
.rank-badge{display:inline-flex;align-items:center;justify-content:center;width:28px;height:28px;border-radius:50%;font-size:0.85rem;font-weight:700;flex-shrink:0;}
.rank-1{background:linear-gradient(135deg,#ffd700,#ffb300);color:#000;box-shadow:0 2px 8px rgba(255,179,0,0.5);}
.rank-2{background:linear-gradient(135deg,#c0c0c0,#a0a0a0);color:#000;box-shadow:0 2px 8px rgba(180,180,180,0.4);}
.rank-3{background:linear-gradient(135deg,#cd7f32,#b06820);color:#fff;box-shadow:0 2px 8px rgba(180,100,30,0.4);}
.rank-other{background:rgba(121,134,203,0.15);color:var(--mut);border:1px solid rgba(121,134,203,0.3);}
.rank-score{font-size:0.65rem;color:var(--mut);margin-right:4px;}
.why-section{background:rgba(41,121,255,0.05);border:1px solid rgba(41,121,255,0.15);border-radius:9px;padding:9px;margin-bottom:8px;}
.why-title{font-size:0.66rem;color:var(--blue);font-weight:600;margin-bottom:6px;}
.why-list{display:flex;flex-wrap:wrap;gap:5px;}
.why-item{background:rgba(0,0,0,0.2);border:1px solid var(--bdr);border-radius:6px;padding:2px 8px;font-size:0.64rem;color:var(--txt);}
.why-item.warn{border-color:rgba(255,179,0,0.3);color:var(--yel);}
/* Glossary */
...
(function(){
  ...
  if(saved && saved!=='tadawul'){
    sessionStorage.removeItem('activeTab');
    document.addEventListener('DOMContentLoaded',function(){switchTab(saved);});
  }
})();
...
function toggle(mkt,i){
  ...
  if(!det){console.log('det not found: det-'+mkt+'-'+i); return;}
...
function filterChip(mkt,ft,el){
  ...
  if(aFilt[mkt]===ft){
    ...
    rows.forEach(r=>{
      ...
      if(ft==='halal'&&r.querySelector('.halal-badge')) show=true;
...
def index():
    ...
    for mkt,label,flag in [("tadawul","السوق السعودي","🇸🇦"),("us","السوق الأمريكي","🇺🇸"),("crypto","العملات","💰")]:
        ...
        if data:
            ...
            if True:
                ...
                if heat:
                    ...
                    for sector, info in heat.items():
                        ...
                        if rank < top_t:
                            bg="rgba(0,230,118,0.12)"; bc="rgba(0,230,118,0.4)"; sc="var(--green)"; label="🔥 الأقوى"
                        elif rank < top_t * 2:
                            bg="rgba(255,179,0,0.1)"; bc="rgba(255,179,0,0.35)"; sc="var(--yel)"; label="👀 متوسط"
                        else:
                            bg="rgba(255,61,87,0.08)"; bc="rgba(255,61,87,0.3)"; sc="var(--red)"; label="❄️ الأضعف"
                        ...
                        out.append('<div class="heat-score" style="color:'+sc+'">Score: '+str(avg)+'</div>')
                        out.append('<div class="heat-bar" style="background:'+sc+';width:'+str(pct)+'%;margin:0 auto;"></div>')
                        out.append('<div class="heat-info">'+label+' | '+str(buy)+' إشارة BUY من أصل '+str(cnt)+' سهم</div>')
                        out.append('</div>')
                    out.append('</div></div>')
            ...
            halal_l=[s for s in buy_l if s.get('halal')]
            out.append('<div class="chip ch" onclick="filterChip(\''+mkt+'\',\'halal\',this)"><div class="num">'+str(len(halal_l))+'</div><div class="lbl">☪️ حلال</div></div>')
            out.append('</div>')
