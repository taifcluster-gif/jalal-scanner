...
<div class="card card-{verdict.lower()}" onclick="toggleCard('{idx}')">
  ...
  <div class="card-detail" id="det-{idx}" style="display:none">
    ...
    <div class="chart-tips">{tips_html}
      <div class="tip-row tip-action">👉 افتح الشارت: إذا السعر قريب من سعر الدخول والشموع خضراء = ادخل. إذا طار بعيد = انتظر فرصة ثانية.</div>
      <a href="{tv_link}" target="_blank" onclick="event.stopPropagation()" class="tv-btn">📊 افتح في TradingView</a>
    </div>
...
<div class="wrap">
  ...
  <!-- محفظتي -->
  <div id="portfolio-section" style="margin-bottom:16px;"></div>
...
// ══ المحفظة ══
var showPfForm = false;
function togglePfForm() {
  showPfForm = !showPfForm;
  var f = document.getElementById('pf-form');
  if (f) f.classList.toggle('show', showPfForm);
}

function savePosition() {
  var code = document.getElementById('pf-code').value.trim().toUpperCase();
  var name = document.getElementById('pf-name').value.trim() || code;
  var market = document.getElementById('pf-mkt').value;
  var entry = parseFloat(document.getElementById('pf-entry').value);
  var tp1 = parseFloat(document.getElementById('pf-tp1').value || 0);
  var sl = parseFloat(document.getElementById('pf-sl').value || 0);
  if (!code || !entry) { showToast('أدخل الرمز وسعر الدخول'); return; }
  fetch('/portfolio/add', {method:'POST', headers:{'Content-Type':'application/json'},
    body: JSON.stringify({code, name, market, entry, tp1, sl})})
  .then(r => r.json()).then(d => {
    if (d.ok) { showToast('تمت الإضافة ✓'); loadPortfolio(); togglePfForm(); }
  });
}

function removePosition(code, market) {
  fetch('/portfolio/remove', {method:'POST', headers:{'Content-Type':'application/json'},
    body: JSON.stringify({code, market})})
  .then(r => r.json()).then(d => { if (d.ok) loadPortfolio(); });
}

function loadPortfolio() {
  fetch('/portfolio').then(r => r.json()).then(positions => {
    var sec = document.getElementById('portfolio-section');
    if (!sec) return;
    if (!positions.length) {
      sec.innerHTML = `<div class="portfolio-wrap">
        <div class="portfolio-header">
          <div class="portfolio-title">📁 صفقاتي المفتوحة</div>
          <button class="portfolio-add-btn" onclick="togglePfForm()">+ أضف صفقة</button>
        </div>
        <div id="pf-form" class="portfolio-form">
          <div class="pf-row">
            <input class="pf-inp" id="pf-code" placeholder="الرمز (مثال: 2222 أو AAPL)">
            <input class="pf-inp" id="pf-name" placeholder="الاسم (اختياري)">
            <select class="pf-inp" id="pf-mkt" style="max-width:110px;">
          ...
          <div class="pf-row">
            <input class="pf-inp" id="pf-entry" type="number" step="any" placeholder="سعر الدخول">
            <input class="pf-inp" id="pf-tp1" type="number" step="any" placeholder="الهدف (اختياري)">
            <input class="pf-inp" id="pf-sl" type="number" step="any" placeholder="وقف الخسارة (اختياري)">
          </div>
          <button class="pf-save" onclick="savePosition()">💾 حفظ الصفقة</button>
        </div>
        <div class="empty-portfolio">لا توجد صفقات مفتوحة — اضغط "+ أضف صفقة" لتتبع أي سهم</div>
      </div>`;
      return;
    }
    var cards = positions.map(pos => {
      var pnlCls = pos.pnl_pct > 0 ? 'up' : (pos.pnl_pct < 0 ? 'down' : '');
      var cardCls = pos.pnl_pct > 2 ? 'winning' : (pos.pnl_pct < -3 ? 'losing' : '');
      var alertHtml = pos.alert ? `<div class="pos-alert">${pos.alert}</div>` : '';
      var sfx = pos.market === 'tadawul' ? '.SR' : '';
      return `<div class="pos-card ${cardCls}">
        <div class="pos-top">
          <div><div class="pos-name">${pos.name} <span class="pos-code">${pos.code}${sfx}</span></div>
            <div style="font-size:0.7rem;color:var(--text3);margin-top:2px;">دخلت: ${pos.date}</div>
          </div>
          <div style="text-align:left;">
            <div class="pos-pnl ${pnlCls}">${pos.pnl_sign}${pos.pnl_pct}%</div>
            <div style="font-size:0.68rem;color:var(--text3);">${pos.status}</div>
          </div>
        </div>
        <div class="pos-details">
          <div class="pos-det">دخول: ${pos.entry}</div>
          <div class="pos-det">الآن: ${pos.current}</div>
          ${pos.tp1 > 0 ? `<div class="pos-det">هدف: ${pos.tp1}</div>` : ''}
          ${pos.sl > 0 ? `<div class="pos-det">وقف: ${pos.sl}</div>` : ''}
        </div>
        ${alertHtml}
        <div style="text-align:left;margin-top:6px;">
          <button class="pos-remove" onclick="removePosition('${pos.code}','${pos.market}')">✕ أغلق الصفقة</button>
        </div>
      </div>`;
    }).join('');
    sec.innerHTML = `<div class="portfolio-wrap">
      <div class="portfolio-header">
        <div class="portfolio-title">📁 صفقاتي المفتوحة (${positions.length})</div>
        <button class="portfolio-add-btn" onclick="togglePfForm()">+ أضف صفقة</button>
      </div>
      <div id="pf-form" class="portfolio-form">
        <div class="pf-row">
          <input class="pf-inp" id="pf-code" placeholder="الرمز">
          <input class="pf-inp" id="pf-name" placeholder="الاسم (اختياري)">
          <select class="pf-inp" id="pf-mkt" style="max-width:110px;">
            <option value="tadawul">🇸🇦 تاسي</option>
            <option value="us">🇺🇸 أمريكي</option>
...
function addStock() {
  ...
  var name = document.getElementById('nName').value.trim();
  var mkt = document.getElementById('nMkt').value;
  if (!code) { showToast('أدخل الرمز'); return; }
...
def index():
    ...
    add_panel = f'''<div class="add-panel">
      ...
      <div class="add-row">
        <input class="add-inp" id="nCode" placeholder="الرمز (مثال: 2222)">
        <input class="add-inp" id="nName" placeholder="الاسم">
    ...
    for mkt, label, flag in [("tadawul","تاسي","🇸🇦"), ("us","الأمريكي","🇺🇸"), ("crypto","العملات","💰")]:
        ...
        if data:
            for i, s in enumerate(data):
                cards_html += build_card(s, f"{mkt}-{i}")
        else:
...
def portfolio_get():
    ...
    for pos in positions:
        current = get_current_price(pos["code"], pos["market"])
        if current:
            entry = pos["entry"]
            pnl_pct = round((current - entry) / entry * 100, 2)
            pnl_sign = "+" if pnl_pct >= 0 else ""
            status = "🟢 رابح" if pnl_pct > 0 else ("🔴 خاسر" if pnl_pct < -3 else "🟡 محايد")
            tp1 = pos.get("tp1", 0)
            sl = pos.get("sl", 0)
            tp1_reached = current >= tp1 if tp1 > 0 else False
            sl_reached = current <= sl if sl > 0 else False
            alert = ""
            if tp1_reached: alert = "🎯 وصل الهدف 1!"
            elif sl_reached: alert = "⚠️ وصل وقف الخسارة!"
            result.append({**pos, "current": current, "pnl_pct": pnl_pct,
                           "pnl_sign": pnl_sign, "status": status, "alert": alert})
        else:
            result.append({**pos, "current": "—", "pnl_pct": 0, "pnl_sign": "", "status": "—", "alert": ""})
...
def portfolio_add():
    ...
    sl = float(d.get("sl", 0))
    if not code or entry <= 0: return jsonify({"ok": False})
    positions = load_portfolio()
    # تحديث لو موجود
    for pos in positions:
        if pos["code"] == code and pos["market"] == market:
            pos.update({"entry": entry, "tp1": tp1, "sl": sl, "name": name or pos["name"],
                       "date": datetime.now().strftime("%Y-%m-%d")})
            save_portfolio(positions)
            return jsonify({"ok": True})
    positions.append({"code": code, "name": name or code, "market": market,
...
def portfolio_remove():
    d = request.get_json()
    code = d.get("code",""); market = d.get("market","")
    positions = [p for p in load_portfolio() if not (p["code"]==code and p["market"]==market)]
    save_portfolio(positions)
    return jsonify({"ok": True})
