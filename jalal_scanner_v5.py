...
def build_card(s, idx):
    ...
    verdict_map = {"BUY": ("buy-badge", "شراء"), "WAIT": ("wait-badge", "انتظر"), "AVOID": ("avoid-badge", "تجنب")}
    badge_cls, badge_txt = verdict_map.get(verdict, ("wait-badge", "انتظر"))
    if "مشروط" in s.get("bt", ""):
        badge_cls = "cond-badge"; badge_txt = "مشروط"

    conf = s.get("confidence", 0)
    conf_cls = "conf-high" if conf >= 70 else ("conf-mid" if conf >= 50 else "conf-low")

    trend_icon = {"استثمار": "📈", "سوينج": "↗️", "مضاربة": "⚡", "تجنب": "⬇️", "انتظر": "→"}.get(s.get("trend", ""), "")
...
<div class="card card-{verdict.lower()}" onclick="toggleCard({idx})">
  <div class="card-top">
    ...
    <div class="card-info">
      <div class="card-name">{s["name"]} <span class="card-code">{s["code"]}{sfx}</span></div>
      <div class="card-meta">{trend_icon} {s.get("trend","")} · {stars}</div>
    </div>
  ...
  <div class="card-summary">
    ...
    <span class="tag">TP +{s["ptp"]}%</span>
    <span class="tag">SL -{s["psl"]}%</span>
    <span class="tag">R:R {s["rr"]}</span>
    <span class="tag">RSI {s["rsi"]}</span>
  ...
  <div class="card-detail" id="det-{idx}" style="display:none">
    <div class="detail-grid">
      <div class="detail-box">
        ...
        <div class="detail-val entry-val" onclick="copyVal(this)">{s["lb"]}</div>
      </div>
      <div class="detail-box">
        <div class="detail-label">هدف 1</div>
        <div class="detail-val tp-val" onclick="copyVal(this)">{s["t1"]} <span class="small-pct">+{s["ptp"]}%</span></div>
      </div>
      <div class="detail-box">
        <div class="detail-label">هدف 2</div>
        <div class="detail-val tp-val" onclick="copyVal(this)">{s["t2"]} <span class="small-pct">+{round(s["ptp"]*2,1)}%</span></div>
      </div>
      <div class="detail-box">
        <div class="detail-label">وقف الخسارة</div>
        <div class="detail-val sl-val" onclick="copyVal(this)">{s["sl"]} <span class="small-pct">-{s["psl"]}%</span></div>
      </div>
    </div>
    ...
    <div class="ma-row">
      <span class="ma-tag {'ma-ok' if s['above_daily'] else 'ma-no'}">يومي {'✓' if s['above_daily'] else '✗'}</span>
      <span class="ma-tag {'ma-ok' if s['above_weekly'] else 'ma-no'}">أسبوعي {'✓' if s['above_weekly'] else '✗'}</span>
      <span class="ma-tag {'ma-ok' if s['above_monthly'] else 'ma-no'}">شهري {'✓' if s['above_monthly'] else '✗'}</span>
    </div>
    <div class="exit-plan">
      <div class="exit-title">📊 خطة الخروج</div>
      <div class="exit-steps">
        <div class="exit-step"><span>عند الهدف 1</span><span class="exit-pct">بع 50%</span></div>
        <div class="exit-step"><span>عند الهدف 2</span><span class="exit-pct">بع 30%</span></div>
...
function copyVal(el) {
  var t = el.textContent.trim().split(' ')[0];
...
def index():
    ...
    html = HTML_TEMPLATE.replace("__SESSION__", sess).replace("__TAB_CONTENTS__", tab_contents)
    return html
...
def add_stock():
    d = request.get_json()
    code = d.get("code","").strip(); name = d.get("name","").strip(); mkt = d.get("market","tadawul")
