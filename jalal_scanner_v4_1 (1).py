...
def run_scan(market):
    ...
    # للسوق الأمريكي: نمسح 100 سهم في كل مرة بالتناوب
    if market=="us" and len(stocks)>100:
        import random,time
        random.seed(int(time.time())//3600)  # يتغير كل ساعة
        items=list(stocks.items())
        random.shuffle(items)
        stocks=dict(items[:100])
...
function startScan(mkt){if(mkt)aTab=mkt;var lb={tadawul:'تاسي',us:'السوق الأمريكي',crypto:'العملات'};document.getElementById('lo').classList.add('show');document.getElementById('lm').textContent=lb[aTab]||aTab;document.getElementById('lo-pfill').style.width='0%';fetch('/scan?market='+aTab).then(r=>r.json()).then(()=>{scanPoll=setInterval(pollProgress,1500);});}
...
def index():
    ...
    for mkt,label,flag in [("tadawul","تاسي","🇸🇦"),("us","السوق الأمريكي","🇺🇸"),("crypto","العملات الرقمية","💰")]:
        ...
        out.append('<div class="scan-wrap"><button class="scan-btn" onclick="startScan(\''+mkt+'\')" id="scan-btn-'+mkt+'">🔍 مسح '+flag+' '+label+'</button>')
        if last:
